...
public class Main_1755 {
	public static void main(String[] args) { // 메인 메서드
		...
		int M = sc.nextInt(); int N = sc.nextInt(); // M, N 입력 받음
		Num[] arr = new Num[N-M+1]; // M이상 N 이하이므로 N-M;
		String[] numE =  new String[] {"zero", "one", "two","three",
				"four","five","six","seven","eight","nine"}; // 영어로 숫자를 만들기위한 배열
		...
		for(Num data : arr) {// 출력을 위한 for-each
			...
			cnt++; // 찍은 수 
			if(cnt%10==0)System.out.println(); // 10마다 줄넘김
		}
		sc.close(); // 스캐너 close
	}
